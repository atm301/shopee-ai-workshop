...
- **index.html**：桌號（第 1 到 7 桌）＋暱稱報到、8 分頁（開始／素材／決策／流程／提示詞／工具／上傳／帶走）、8 題現場互動（含五題互動題投完即看解答）、開場與收尾的時間差計算機、18 張提示詞一鍵複製直開 ChatGPT／Gemini（含《提示詞總表》官方 11 張，可一次複製全部或列印）、36 個工具連結（自家網域全帶 UTM）、分組作業上傳（文字或圖片，append-only 多版全留）、各桌即時分數、7 天行動清單與列印卡。
